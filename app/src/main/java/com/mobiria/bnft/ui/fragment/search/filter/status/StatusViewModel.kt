...
class StatusViewModel(val mActivity: BaseActivity, item: ODataCategory, val mCallBack: CategoryClickEvent) : ViewModel {
    ...
    fun onAuctionsClick(view: View){
        isTrue.set(2)
        FilterData.status = "bid"
    }
    fun onForSaleClick(view: View){
        isTrue.set(3)
        FilterData.status = "buy"
    }
